//IMPLEMENTACIÓN DE ÁRBOL GENERAL MEDIANTE LISTAS DE HIJOS
...
void Agen<T>::insertarRaiz(const T& e) {
    ...
    numNodos = 1;
    nodos[0].elto = e; // la lista de hijos está vacía
}
...
void Agen<T>::insertarHijoIzqdo(Agen<T>::nodo n, const T& e) {
    ...
    for (hizqdo = 1; nodos[hizqdo].padre != NODO_NULO; hizqdo++); // añadir el nuevo nodo en la primera celda libre
    nodos[hizqdo].elto= e;
    nodos[hizqdo].padre = n; // insertar el nuevo nodo al inicio de la lista de hijos de n
